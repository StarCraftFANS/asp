...
static void print_ihex_rec(const IHEX_RECORD *ihex_rec);
...
static void print_ihex_rec(const IHEX_RECORD *ihex_rec)
{
  // Print Intel HEX record information
  printf("---- INTEL HEX RECORD :\n");
  printf("  nbytes = 0x%02x\n", ihex_rec->nbytes);
  printf("  addr   = 0x%04x\n", ihex_rec->addr);
  printf("  type   = 0x%02x\n", ihex_rec->type);
